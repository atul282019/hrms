var uploadVoucherIcon_base64="";
var uploadMccIcon_base64="";
var uploadMccMainIcon_base64=""
...
function saveVoucherData() {
    // Gather form data
    var voucherId = 
	document.getElementById("VoucherId").value.trim();
    var voucherName = 
	document.getElementById("voucherName").value.trim();
    var purposeCode = 
	document.getElementById("purposeCode").value.trim();
    var purposeDesc =
	document.getElementById("purposeDesc").value.trim();
	
    var mccCode = 
	document.getElementById("mccCode").value.trim();
    var mccDesc = 
	document.getElementById("mccDesc").value.trim();
	var status = 
	document.getElementById("status").value.trim();

    // Convert image files to Base64 if provided
    var voucherIconBase64 = getBase64Image("#uploadVoucherIcon");
    var mccIconBase64 = getBase64Image("#uploadMccIcon");
    var mccMainIconBase64 = getBase64Image("#uploadMccMainIcon");

    // Validate inputs
    /*if (!voucherName) {
        $("#voucherNameerror").text("Please enter Voucher Name");
        return;
    } else {
        $("#voucherNameerror").text("");
    }

    if (!purposeCode) {
        $("#purposeCodeError").text("Please enter Purpose Code");
        return;
    } else {
        $("#purposeCodeError").text("");
    }

    if (!purposeDesc) {
        $("#purposeDescError").text("Please enter Purpose Description");
        return;
    } else {
        $("#purposeDescError").text("");
    }

    if (!mccCode) {
        $("#mccCodeError").text("Please enter MCC Code");
        return;
    } else {
        $("#mccCodeError").text("");
    }
    ...
    if (!mccDesc) {
        $("#mccDescError").text("Please enter MCC Description");
        return;
    } else {
        $("#mccDescError").text("");
    }*/
