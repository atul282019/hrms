...
async function  issueVoucher(){
	...
	const tableBody = document.getElementById('voucherTableBody');
	   ...
	   const rows = tableBody.rows[0]; // or tableBody.children[0]
	    //console.log(firstRow.innerHTML); //
	    const tableData = [];
	
	  //  rows.forEach((row, index) => {
	   
	        const nameInput = rows.querySelector('input[placeholder="Enter Name"]');
	        const mobileInput = rows.querySelector('input[placeholder="Enter Mobile Number"]');
	        const voucherInput = rows.querySelector('.table-input-voucher');
			const selectMCC = rows.querySelector('.selectMCC');
			const selectPurpose = rows.querySelector('.selectPurpose');
			const selectMCCDescription = rows.querySelector('.selectMCCDescription');
			const selectPurposeDescription = rows.querySelector('.selectPurposeDescription');
	        const redemptionSelect = rows.querySelector('.redemptionType');
	        const amountInput = rows.querySelector('input[placeholder="Enter Amount"]');
	        const dateInput = rows.querySelector('input[type="date"]');
	        const validitySelect = rows.querySelector('.validity');
